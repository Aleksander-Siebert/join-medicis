...
1. Repère le **slug** du terme dans [`src/lib/glossary.ts`](../../src/lib/glossary.ts)
   (le champ `slug`, ex. `modele-de-langage`).
2. Dans ce dossier, clique sur **Add file → Create new file**.
3. Nomme le fichier **exactement** `<slug>.md` (ex. `modele-de-langage.md`).
...
|---|---|---|
| `excerpt` | non | Chapô sous le titre + meta description. **Fortement recommandé** pour le SEO. |
| `updatedAt` | non | Date `AAAA-MM-JJ`, affichée en bas de l'article. |
| `title` | non | Remplace le titre du terme. À n'utiliser que si le titre du glossaire ne convient pas. |
